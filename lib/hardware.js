...
const defaultStatus = _.mapValues(hardwareIds, (value, name) => {
  return {
    name,
    level: levels.STALE,
    homed: false
...
class Hardware extends EventEmitter2 {
  ...
  _resetHardwareLater = _.debounce(this._resetHardware, HARDWARE_TIMEOUT);
  _resetHardware() {
  ...
  _resetBatteryLater = _.debounce(this._resetBattery, BATTERY_TIMEOUT);
  _resetBattery() {
  ...
  _onEbuttons(msg) {
    const status = _.map(msg.status, status => {
  ...
  _resetEbuttonsLater = _.debounce(this._resetEbuttons, EBUTTONS_TIMEOUT);
  _resetEbuttons() {
...
function diagnosticMsgToStatus(msg) {
  const parts = msg.status.map(part => {
    return {
      name: part.name,
      level: part.level,
  ...
  const hardware_status = _.indexBy(parts, 'name');

  // fill all missing hardware parts with 'idle'
  _.defaults(hardware_status, defaultStatus);

  _.mapValues(hardware_status, part => {
...
function getActions(part) {
  const props = properties[part.name];
  if (!props) {
    return null;
  }
