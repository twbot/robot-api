...
const defaultStatus = mapValues(hardwareIds, (value, name) => {
  return {
    name,
    level: levels.STALE,
    homed: false
...
function diagnosticMsgToStatus(msg) {
  ...
  const hardware_status = indexBy(parts, 'name');

  // fill all missing hardware parts with 'idle'
  defaults(hardware_status, defaultStatus);

  mapValues(hardware_status, part => {
