...
class Head extends EventEmitter2 {
  ...
  getImage(width = 128, callback) {
    ...
    this.getImageService.callService(request, response => {
      ...
      const rgb_base_64 = fromByteArray(rgb_data.buffer);
      const depth_base_64 = fromByteArray(depth_data.buffer);

      const rgb_image_url = `data:image/jpeg;base64,${rgb_base_64}`;
      const depth_image_url = `data:image/jpeg;base64,${depth_base_64}`;
